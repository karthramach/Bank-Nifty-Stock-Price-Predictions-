...
training_set = dataset_train.iloc[:, [1,2,4]].values
...
for i in range(100, 2500):
    X_train.append(training_set_scaled[i-100:i, 0:3])
...
regressor.add(LSTM(units = 50,input_shape = (X_train.shape[1], 3)))
...
dataset_total = pd.concat((dataset_train[['Open','High','Close']], dataset_test[['Open','High','Close']]), axis = 0, ignore_index=True)
inputs = dataset_total[len(dataset_total) - len(dataset_test) - 100:].values
inputs = inputs.reshape(-1,3)
inputs = sc.transform(inputs)
X_test = []
for i in range(100, 148):
    X_test.append(inputs[i-100:i,0:3])
X_test = np.array(X_test)
X_test = np.reshape(X_test, (X_test.shape[0], X_test.shape[1], 3))
predicted_stock_price = regressor.predict(X_test)
predicted_stock_price_new = np.zeros(shape=(len(predicted_stock_price), 3))
